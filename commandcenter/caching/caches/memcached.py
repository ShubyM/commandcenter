...
class MemcachedCache(MemoCache):
    ...
    def _ping(self) -> None:
        try:
            ...
            _LOGGER.error("Disabling memcached cache", exc_info=True)
            self._enabled = False
        else:
            if not pong:
                _LOGGER.error("Disabling memcached cache, server did not return PONG")
                self._enabled = False
    ...
    def _remove_from_memcached(self, value_key: str) -> None:
        ...
        key = self._get_memcached_key(value_key)
        try:
            deleted = self.memcached.delete(key)
        except Exception:
